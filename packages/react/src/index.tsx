...
export const TourProvider = ({
  ...
  const [events, setEvents] = useState<EventBus<FlowEvents<ReactNode>> | null>(null)
  ...
  const ensureStore = useCallback(
    (flowId: string): FlowStore<ReactNode> => {
      ...
      const store = createFlowStore(definition, {
        storageAdapter,
        storageKey: storageNamespace ? `${storageNamespace}:${definition.id}` : undefined,
        persistOnChange,
      })
  ...
  const getActiveStore = useCallback((): FlowStore<ReactNode> => {
    ...
    if (!store) {
      throw new Error('No active flow. Call startFlow before controlling progression.')
    }
    return store
  }, [])
  ...
  const goToStep = useCallback((step: number | string) => getActiveStore().goToStep(step), [getActiveStore])
  const pause = useCallback(() => getActiveStore().pause(), [getActiveStore])
  const resume = useCallback(() => getActiveStore().resume(), [getActiveStore])
  const cancel = useCallback((reason?: string) => getActiveStore().cancel(reason), [getActiveStore])
  const complete = useCallback(() => getActiveStore().complete(), [getActiveStore])
  ...
  const contextValue: TourContextValue = useMemo(
    ...
    [activeFlowId, activeStep, back, cancel, complete, events, flowMap, goToStep, next, pause, resume, startFlow, state],
  )
...
export const TourRenderer = ({ flowId, children, fallback = null }: TourRendererProps) => {
...
export const useTourEvents = <TEventKey extends Extract<keyof FlowEvents<ReactNode>, string>>(
  event: TEventKey,
