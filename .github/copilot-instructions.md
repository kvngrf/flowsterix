# Copilot Instructions for Flowster

## Architecture Overview

Flowster is an onboarding tour/flow library with a framework-agnostic core and React bindings. The monorepo uses **pnpm workspaces + Turborepo**.

### Package Structure
- `@tour/core` – Framework-agnostic state machine, flow definitions (Zod-validated), persistence, event bus. Zero React dependencies.
- `@tour/react` – React bindings with `TourProvider`, hooks (`useTour`, `useTourTarget`, `useTourHud`), UI components, router adapters, and motion/animation system.
- `@tour/headless` – Re-exports `@tour/react` minus UI components/CSS for custom HUD implementations.
- `@tour/themes` – Token presets for theming.
...
- Import `@tour/react/styles.css` once in app entry
- Use `data-tour-*` attributes for custom styling hooks
- Tailwind classes in `@tour/react` components use `tailwind-merge`

### Router Integration
Multiple adapters at `@tour/react/router/*`:
